...
class TestInputCommand(unittest.TestCase):
    ...
    def test_validate_valid_input_flag1(self):
        ...
        self.assertEqual(command.validate_input_flag(InputFlag('test', value=None, status=None)), ValidationStatus.VALID)

    def test_validate_valid_input_flag2(self):
        command = Command('some', flags=Flags([Flag('test'), Flag('more')]))
        self.assertEqual(command.validate_input_flag(InputFlag('more', value=None, status=None)), ValidationStatus.VALID)

    def test_validate_undefined_input_flag1(self):
        command = Command('some', flags=Flag('test'))
        self.assertEqual(command.validate_input_flag(InputFlag('more', value=None, status=None)), ValidationStatus.UNDEFINED)

    def test_validate_undefined_input_flag2(self):
        command = Command('some', flags=Flags([Flag('test'), Flag('more')]))
        self.assertEqual(command.validate_input_flag(InputFlag('case', value=None, status=None)), ValidationStatus.UNDEFINED)
    ...
    def test_invalid_input_flag1(self):
        ...
        self.assertEqual(command.validate_input_flag(InputFlag('test', value='example', status=None)), ValidationStatus.INVALID)

    def test_invalid_input_flag2(self):
        command = Command('some', flags=Flag('test', possible_values=['some', 'case']))
        self.assertEqual(command.validate_input_flag(InputFlag('test', value='slay', status=None)), ValidationStatus.INVALID)
